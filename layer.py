from write_eclipse import Writer
import numpy as np
...
class LobeLayer(Layer):
    ...
    def create_geology(self, dhmax=4, dhmin=4, rmin=42, rmax=44, asp=1.5, theta0=0, m=100):

        np.random.seed(20)
        allfacies, allporo, allsurface = self.lobemodeling(dhmax=dhmax, dhmin=dhmin, rmin=rmin, rmax=rmax, asp=asp, theta0=theta0, m=m)     
        ...
        self.active = self.active > np.percentile(self.active.flatten(), self.ntg*100)
    ...
    def lobemodeling(self, dhmax=4, dhmin=4, rmin=42, rmax=44, asp=1.5, theta0=0, m=100):
        ...
        while(i < iiii-1):
            ...
            if i > start-0.1:
                allsurface.append(surface.copy())
                self.assign_prop(xchange, ychange, x, y, theta, surface0, surface, facies, r, poro, dz, asp, i-start+1) 
            i+=1
    ...
    def assign_prop(self, xchange, ychange, x, y, theta, surface0, surface, facies, r, poro, dz, asp, i):
        for n in range(xchange.size):
            ...
            if top > bot:
                ...
                for kk in np.arange(bot, top):
                    ...
                    poro[kk,ii,jj] = (((top-kk)/(top-bot))**1) * 0.3 * (1-(r1/r)) * ((1-kk/self.nz)/2+0.5) + 0.05
